...
impl Node {
    ...
    // Adjusted chunking and distribution method to handle parity chunks
    pub fn chunk_and_distribute_file(&self, file_path: &str, chunk_size: chunk::ChunkSize) -> io::Result<Uuid> {
        ...
        // Chunk the file and generate parity chunks
        ...
        // Distribute regular chunks among peers
            ...
            for i in (chunk_index..chunk_collection.num_chunks).step_by(peers_count) {
                ...
                if let Ok(mut chunk_file) = File::open(&chunk_path) {
                    ...
                    match self.connect_to_peer(peer, Message::ChunkResponse {
                        ...
                        is_parity_chunk: false,  // This is a regular data chunk
                    }) {
        ...
        // Send parity chunk to all peers
        ...
        if let Ok(mut parity_chunk_file) = File::open(&parity_chunk_path) {
            ...
            // Send the parity chunk to all peers
            for peer in &self.peers {
                match self.connect_to_peer(peer, Message::ChunkResponse {
                    file_uuid: chunk_collection.uuid,
                    chunk_index: chunk_collection.num_chunks, // Use the next chunk index for parity
                    chunk_data: buffer.clone(),
                    total_chunks,
                    parity_chunks: num_parity_chunks,
                    is_parity_chunk: true,  // This is a parity chunk
                }) {
            ...
            // Store the parity chunk locally
            self.store_chunk(
                chunk_collection.uuid,
                chunk_collection.num_chunks, // Use the next chunk index for parity
                buffer,
                total_chunks,
                num_parity_chunks,
                true,  // Mark this as a parity chunk
            );
            info!("Stored parity chunk locally");
        }

        // Store remaining chunks (if any) locally
    ...
    if let Some(file) = files.get(&file_uuid) {
        ...
        self.request_missing_chunks(file_uuid, file.total_chunks - file.parity_chunks, files.clone())?;
        thread::sleep(Duration::from_secs(3));  // Allow time for missing chunks to arrive

        // Check for the total number of chunks (including the parity chunk)
        ...
        // Check if we have all the chunks, including the parity chunk
